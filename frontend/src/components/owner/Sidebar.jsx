    ...
    <div className='relative min-h-screen md:flex flex-col items-center pt-8 max-w-12 md:max-w-60 w-full border-r border-borderColor text-sm'>
      <div className='group relative'>
        <label htmlFor="image">
            ...
            <div className='absolute hidden top-0 right-0 left-0 border-0 bg-black/10 rounded-full group-hover:flex items-center justify-center cursor-pointer'>
      ...
      <div className="w-full">
        {ownerMenuLinks.map((link,index)=>(
            <NavLink key={index} to={link.path} className={`flex relative items-center gap-2 w-full py-3 pl-4 first:mt-6 ${link.path === location.pathname ? 'bg-primary/10 text-primary' : 'text-gray-600'}`}>
